...
class Cube:
    def __init__(self, pos, size=1, color="white"):
        ...
        points.append(Point(pos) + (0.01*size, 0, 0))
        points.append(Point(pos) + (0.01*size, 0, size))
        points.append(Point(pos) + (0, 0, size))

        points.append(Point(pos) + (0, 2*size, 0))
        points.append(Point(pos) + (0.01*size, 2*size, 0))
        points.append(Point(pos) + (0.01*size, 2*size, size))
        ...
        for cover in covers:
            ...
            for cover_point, middle_normal, middle_moved in zip(
                ...
                divided_covers.append(
                    (cover_point, middle_normal, center, middle_moved)
                )
